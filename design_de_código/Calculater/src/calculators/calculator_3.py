...
from src.drivers.interfaces.drive_handler_interface import DriveHandleInterface
from src.errors.http_unprocessable_entity import HttpUnprocesableEntityError
from src.errors.http_bad_request import HttpBadRequestError

class Calculator3:
    def __init__(self, driver_handler: DriveHandleInterface) -> None:
    ...
    def __verify_results(self, variance: float, multiplication: float) -> None:
        if variance < multiplication:
            raise HttpBadRequestError('Falha no precesso: Variância menor que multiplicação')
